  ...
  function handleSignup(event) {
    ...
    const criteria = {
      ...
      password: [
        ...
        [
          /(?=[^0-9a-zA-Z])/,
          "Password must contain non-alphanumeric characters.",
        ],
    ...
    function findErrors(category) {
      let errorsFound = criteria[category]
        .filter((errorType) => !userInfo[category].match(errorType[0]))
        .map((errorType) => errorType[1]);

      if (errorsFound.length > 0) {
        inputsHaveErrors = true;
      }

      document.getElementById(category + "Errors").innerHTML =
        errorsFound.join("<br>");
    ...
    if (!inputsHaveErrors) {
      ...
      console.log(
        "Note: if username is taken, you will get a 400 response code."
      );

      axios
        .post(
          "https://everywherefitness-9ab34b5731c6.herokuapp.com/api/auth/register",
        ...
        .catch((response) => {
          ...
          errorInfo.signup.push(
            "Couldn't access database. Username may have been taken."
          );
